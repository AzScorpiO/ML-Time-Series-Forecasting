...
# 创建RNN模型
model = Sequential()
model.add(SimpleRNN(100, return_sequences=True, input_shape=(time_step, X.shape[2])))
...
# 记录程序结束时间并计算运行时间
end_time = time.time()
rnn_100_execution_time = end_time - start_time
print(f"RNN运行时间为: {rnn_100_execution_time} 秒")

# 将rnn_execution_time输出到rnn_execution_time.txt文件
with open('rnn_100_execution_time.txt', 'w') as f:
    f.write(f"{rnn_100_execution_time}")    
...
# 将wape_value输出到mlp_wape.txt文件
with open('rnn_100_wape.txt', 'w') as f:
