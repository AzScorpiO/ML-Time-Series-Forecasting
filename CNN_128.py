...
# 创建CNN模型
model = Sequential()
model.add(Conv1D(filters=128, kernel_size=2, activation='relu', input_shape=(time_step, X.shape[2]))) 
model.add(Flatten())
...
# 记录程序结束时间并计算运行时间
end_time = time.time()
cnn_128_execution_time = end_time - start_time
print(f"CNN运行时间为: {cnn_128_execution_time} 秒")

# 将cnn_execution_time输出到cnn_execution_time.txt文件
with open('cnn_128_execution_time.txt', 'w') as f:
    f.write(f"{cnn_128_execution_time}")
...
with open('cnn_128_wape.txt', 'w') as f:
